...
const updateConsentMode = () => {
  if (typeof window === 'undefined' || !window.cookieyes || typeof window.cookieyes.getConsent !== 'function' || !window.gtag) return;
...
export const trackPageView = (url: string) => {
  ...
  const consent = typeof window.cookieyes?.getConsent === 'function' ? window.cookieyes.getConsent() : null;
